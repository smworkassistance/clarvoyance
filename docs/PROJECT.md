...
- **`nav_v2`** (ON BY DEFAULT since v249; kill switch = `feature_flags.nav_v2 enabled=false`; device: `?nav=0` off / `?nav=2` on / `?nav=reset`): new 6-button bottom bar (Feed / Vibe / Clar AI raised centre / Goal / Home / You). Activate for everyone: `insert into feature_flags(key,enabled) values ('nav_v2', true) on conflict (key) do update set enabled=true;` — deactivate: set `enabled=false`. Per-device dev override: `?nav=2` on, `?nav=0` off. Feed = Community hosted as a tab (sub-tabs Following/Discover/Board); You = the Community `me` dashboard hosted as a tab (⚙ → App Profile, private Fortune card, Board rank card → Leaderboard page, goal plates with per-goal share; v248); Self plate sits under Non-Negotiables on Home. All code is one self-contained block (`#navv2-css`/`#navv2-js`) at the end of the HTML; it never removes the old nav.
