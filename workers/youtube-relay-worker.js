  ...
  async function growTopicPool(env, topic, query) {
    ...
    /* v205a: fixed a real bug found live in production — a topic cached
      BEFORE this pagination system existed (e.g. "discipline", already
      full of 12 videos from the old worker) has no state row, and the
      previous version of this function just called getVideosForTopic()
      here, which no-ops on any cache hit — meaning it NEVER actually
      progressed to a real page-2 search or ever created a state row, so
      every `more:true` call for a pre-existing topic silently returned
      the same 12 forever, permanently, with no way to recover. There is
      no stored nextPageToken to resume from for a topic like this (it
      predates this table), so the only correct fix is to run one real
      fresh page-1 search now — it's fine if some of those 12 results
