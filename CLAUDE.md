...
| PWA | sw.js (cache version clv-v177), manifest.json, beforeinstallprompt — **bump CACHE_VERSION on every release** |
...
```
- `index.html` = always the latest pushed stable/testing version (what users see on GitHub Pages) — currently **v181**
...
*Last updated: 2026-08-08 — admin.html fix: charger category dropdown was silently trimming "Monk mode"'s real id ("Monk " with trailing space), orphaning 2 real chargers from the app entirely. index.html still at v181. Always copy new version to index.html after pushing.*
